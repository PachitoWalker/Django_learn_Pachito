...
def profile(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f"{request.user.username}, ваши изменения успешно сохранены!")
            return HttpResponseRedirect(reverse('user:profile'))
...
@login_required
def logout(request):
    messages.success(request, f"{request.user.username}, будем ждать вашего возвращения!")
    auth.logout(request)
